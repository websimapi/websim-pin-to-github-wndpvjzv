(() => {
  ...
  function installWebsimSessionBridge() {
    ...
    api.runtime.sendMessage({ type:'REQUEST_WEBSIM_SESSION' }).then((result) => { if (result?.injected) return; const script=document.createElement('script'); script.src=api.runtime.getURL('page-bridge.js'); script.onload=script.onerror=() => script.remove(); (document.head || document.documentElement).appendChild(script); }).catch((error) => debug('content.session.bridge.error', { message:error.message }));
  }
