(() => {
  ...
  const defaults = { enabled:true, token:'', owner:'', branchMode:'main', customBranch:'', visibility:'private', visibilityByContext:{}, lastEvents:[], syncedVersions:{}, projectMap:{}, debugLogs:[], advancedLogs:true, advancedLogsConfigured:false };
  ...
  function ownershipForProject(project, projectId, tabId) {
    const session=websimSessions.get(normalizedTabId(tabId)), creator=projectCreator(project);
    if (!session) return { allowed:false, skipped:'websim-user-unknown', message:'Could not verify the signed-in Websim user, so syncing is disabled for safety.' };
    if (!creator) return { allowed:false, skipped:'project-owner-unknown', message:'Could not verify this project\'s creator, so syncing is disabled for safety.' };
    const idsMatch=Boolean(session.id && creator.id && session.id === creator.id), namesMatch=Boolean(session.username && creator.username && session.username.toLowerCase() === creator.username.toLowerCase());
  ...
  async function readWebsimSession(tabId) { const scope=normalizedTabId(tabId); if (scope === null || !api.scripting?.executeScript) return null; const results=await api.scripting.executeScript({ target:{ tabId:scope }, world:'MAIN', func:async () => { let runtimeUser=null; try { if (window.websim?.getUser) runtimeUser=await window.websim.getUser(); } catch {} const root=window.__remixContext?.state?.loaderData?.root, auth=root?.authUser, profile=root?.user, user={ id:auth?.id ?? profile?.id ?? runtimeUser?.id, email:auth?.email ?? runtimeUser?.email, username:profile?.username ?? runtimeUser?.username, avatar_url:profile?.avatar_url ?? runtimeUser?.avatar_url }; return user.id || user.username ? { id:user.id, username:user.username } : null; } }); return normalizedWebsimUser(results?.[0]?.result); }
  ...
  function scheduleReadinessRetry(payload, tabId) {
    ...
    if (pending?.timer) { pending.payload={ ...pending.payload, ...payload }; return; }
    const attempt=pending?.attempt || 0, delay=readinessRetryDelays[Math.min(attempt, readinessRetryDelays.length - 1)];
    const retry={ attempt:attempt + 1, payload:{ ...payload }, timer:setTimeout(() => {
      readinessRetryTimers.set(key, { attempt:retry.attempt, payload:retry.payload, timer:null });
      autoSyncNewProject(retry.payload, tabId).then((result) => { if (!result?.skipped) notify(tabId,result); }).catch((error) => debugLog('sync.page-ready.failed', { tabId:normalizedTabId(tabId), projectId, message:error.message }));
    }, delay) };
  ...
  async function autoSyncNewProject(payload, tabId) {
    ...
    try {
      ...
      if (!ownership.allowed) { await debugLog('sync.page-ready.skipped', { projectId, tabId:normalizedTabId(tabId), reason:ownership.skipped, creator:ownership.creator?.username || null }); return { ok:true, skipped:ownership.skipped, projectId, message:ownership.message }; }
      const requestedVersion=payload?.requestedVersion;
      if (requestedVersion !== undefined && requestedVersion !== null && (version === null || Number(version) < Number(requestedVersion))) {
        await debugLog('sync.page-ready.waiting', { projectId, tabId:normalizedTabId(tabId), reason:'requested-version-not-current', version, requestedVersion });
  ...
  async function ensureRepository(payload, revision, settings, tabId) {
    const user = await gh('/user', settings.token), owner = user.login, mapped = settings.projectMap?.[payload.projectId], visibility = visibilityForContext(settings, payload.projectId, tabId);
    ...
    if (!repository) {
      repository = await gh('/user/repos', settings.token, { method:'POST', body:JSON.stringify({ name, description:`Websim backup for ${payload.title || payload.projectId}`, private:visibility !== 'public', auto_init:false }), headers:{'Content-Type':'application/json'} }); created = true;
    }
  ...
  async function sync(payload, tabId) {
    ...
    try {
      ...
      if (syncInFlight.has(runKey)) { pendingSyncs.set(runKey, { payload:{ ...payload, projectId }, tabId }); await debugLog('sync.queued-in-flight', { projectId, tabId:normalizedTabId(tabId) }); return { ok:true, inProgress:true, skipped:'in-progress', queued:true, message:'This project is already being synced; the newest revision will follow' }; }
      ...
      const target = await ensureRepository({ ...payload, projectId, title:payload.title || readiness.project.title || null, slug:readiness.project.slug || null }, revision, settings, tabId);
    ...
    finally { if (projectId) { const runKey = syncKey(projectId, tabId); syncInFlight.delete(runKey); activeSyncs.delete(runKey); setSyncIndicator(activeSyncForTab(tabId), tabId); const pending=pendingSyncs.get(runKey); if (pending) { pendingSyncs.delete(runKey); setTimeout(() => { debugLog('sync.pending.recheck', { projectId, tabId:normalizedTabId(tabId) }); autoSyncNewProject(pending.payload, pending.tabId).then((result) => { if (!result?.skipped) notify(pending.tabId,result); }).catch((error) => debugLog('sync.page-ready.failed', { tabId:normalizedTabId(pending.tabId), projectId, message:error.message })); }, 0); } } }
  }
  ...
  api.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message?.type === 'GET_WEBSIM_SESSION') {
      const tabId=normalizedTabId(message.tabId);
      if (sender.tab || !isFirstPartyWebsimUrl(message.url)) { sendResponse({ ok:true, user:null }); return; }
      readWebsimSession(tabId).then((user) => {
        if (user) websimSessions.set(tabId, user); else websimSessions.delete(tabId);
        debugLog('websim.session.popup.read', { tabId, username:user?.username || null, signedIn:Boolean(user) });
