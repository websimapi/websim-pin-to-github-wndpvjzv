...
The extension creates one repository per Websim project, using a readable title plus a short project ID suffix. It stores the token in `browser.storage.local` and calls Websim’s revision/CDN endpoints and GitHub’s Git Data API directly from the browser. If a sync fails, open the popup and use **Copy logs** to copy the sanitized diagnostic trail.
